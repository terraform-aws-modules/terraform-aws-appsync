...
locals {
  service_roles_with_policies = (var.create_graphql_api || var.create_event_api) ? { for k, v in var.datasources : k => v if contains(["AWS_LAMBDA", "AMAZON_DYNAMODB", "AMAZON_ELASTICSEARCH", "AMAZON_OPENSEARCH_SERVICE", "AMAZON_EVENTBRIDGE", "RELATIONAL_DATABASE"], v.type) && tobool(lookup(v, "create_service_role", true)) } : {}
...
resource "aws_iam_role" "logs" {
  count = (var.create_graphql_api || var.create_event_api) && var.logging_enabled && var.create_logs_role ? 1 : 0
